...
function AddBook() {
        ...
        .then((result) => {
          if (result.isConfirmed) {
            ...
            const response = axios.delete(
              `http://localhost:3000/admin/delete-book/${bookId}`,
              {
                headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3000/admin/addbook/${uID}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("jwt_token")}`,
  ...
  useEffect(() => {
    ...
    if (token) {
      axios
        .get("http://localhost:3000/user/get-all-books", {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((response) => {
          if (response.data.ok) {
            ...
            <div className="card book-form-card shadow-lg">
              <div className="card-body">
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mb-4">
                    <label htmlFor="bookPhoto" className="form-label">
                      Book Photo
                    </label>
                    <input
                      type="file"
                  ...
                  <div className="d-grid">
                    <button
                      type="submit"
                      className="btn btn-primary btn-add-book"
                    >
                      Add Book
                    </button>
                  </div>
